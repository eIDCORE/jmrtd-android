...
public class EACTAProtocol {
     ...
     */
    public static byte[] deriveIdentifier(PACEResult paceResult) throws NoSuchAlgorithmException {
        ...
        if ("DH".equals(agreementAlg)) {
            ...
            ECPublicKeyParameters ecPublicKeyParameters = Util.toBouncyECPublicKeyParameters((ECPublicKey) pcdPublicKey);
            byte[] t = Util.i2os(ecPublicKeyParameters.getQ().getAffineXCoord().toBigInteger());
            return Util.alignKeyDataToSize(t, (int)Math.ceil(ecPublicKeyParameters.getParameters().getCurve().getFieldSize() / 8.0));
